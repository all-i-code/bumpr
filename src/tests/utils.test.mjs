...
jest.unstable_mockModule('cosmiconfig', () => ({cosmiconfig: jest.fn()}), {virtual: true})
jest.unstable_mockModule('fs', () => ({readFileSync: jest.fn()}), {virtual: true})
jest.unstable_mockModule('../logger.mjs', () => ({error: jest.fn(), log: jest.fn()}), {virtual: true})
...
describe('getChangelogForPr()', () => {
  ...
  describe('when changelog section is present', () => {
    beforeEach(() => {
      pr.description = '##changelog\r\n## Fixes\nFoo, Bar, Baz\n## Features\nFizz, Bang'
      changelog = getChangelogForPr(pr, [])
    })
  ...
  describe('when required present', () => {
    ...
    describe('when changelog matches pattern', () => {
      beforeEach(() => {
        ...
        changelog = getChangelogForPr(pr, ['DEV-\\d+'])
      })
...
describe('getScopeForPr()', () => {
  ...
  it('should return scope when a single version-bump is present', () => {
    ...
    expect(getScopeForPr(pr)).toBe('minor')
  })
...
You can disable automated security fix PRs for this repo from the [Security Alerts page](https://github.com/all-i-code/bumpr/network/alerts).
    ...
    expect(getScopeForPr(pr)).toBe('patch')
  })
  ...
  describe('when given a maxScope', () => {
    ...
    it('should return the scope when less than max scope', () => {
      expect(getScopeForPr(pr, 'major')).toBe('minor')
    })

    it('should return the scope when same as max scope', () => {
      expect(getScopeForPr(pr, 'minor')).toBe('minor')
    })

    it('should throw error when greater than max scope', () => {
      expect(() => {
        getScopeForPr(pr, 'patch')
...
describe('maybePostComment()', () => {
  ...
  describe('when feature is enabled and isPr is true (and no SKIP_COMMENTS is present)', () => {
    ...
    beforeEach(() => {
      config.isEnabled.mockImplementation(name => name === 'comments')
      promise = maybePostComment(config, vcs, 'fizz-bang')
        .then(resp => {
          result = resp
        })
        .catch(err => {
  ...
  describe('when feature is enabled and isPr is true, and isError is true', () => {
    ...
    beforeEach(() => {
      config.isEnabled.mockImplementation(name => name === 'comments')
      promise = maybePostComment(config, vcs, 'fizz-bang', true)
        .then(resp => {
          result = resp
        })
        .catch(err => {
...
describe('maybePostCommentOnError()', () => {
  ...
  describe('when func succeeds', () => {
    beforeEach(() => {
      ...
      return maybePostCommentOnError(config, vcs, func)
        .then(resp => {
          result = resp
        })
        .catch(err => {
  ...
  describe('when func throws an error', () => {
    ...
    describe('and prComments is true and isPr is true', () => {
      ...
      beforeEach(() => {
        config.computed.ci.isPr = true
        config.isEnabled.mockImplementation(name => name === 'comments')
        promise = maybePostCommentOnError(config, vcs, func)
          .then(resp => {
            result = resp
          })
          .catch(err => {
